...
Kokoro speaks three utterances into the bridge WebSocket in real time (like a browser mic):
  1. a factual question     -> brain should answer, PersonaPlex should SAY it
  2. a reasoning question   -> brain should answer
  3. small talk             -> brain should PASS
...
UTTS = [
    ...
    "Do you think the Dallas Cowboys are going to play well this year?",
    "If a train leaves at two fifteen and the trip takes one hour fifty, what time does it arrive?",
    "Cool, thanks. I'm having a pretty relaxed morning.",
]
...
async def main():
    async with aiohttp.ClientSession() as s:
        qs = [await kokoro(s, u) for u in UTTS]
        audio, events, texts = [], [], []
        ready = asyncio.Event()
        t0 = time.monotonic()
